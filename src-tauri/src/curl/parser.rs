...
// Expands --flag=value into ["--flag", "value"] so the main parser doesn't
// need to handle the = form in every arm.
...
fn parse_tokens(tokens: Vec<String>) -> ParsedCurl {
    ...
    while let Some(tok) = iter.next() {
        match tok.as_str() {
            ...
            // Combined short flags, e.g. -XPOST, -vL, -sH "Header: val"
...
// Names that are cookie attributes, not actual cookie key=value pairs.
// These appear in Set-Cookie headers and sometimes get copied into -b arguments.
    ...
    I: Iterator<Item = String>,
{
    ...
    while i < chars.len() {
        match chars[i] {
            'X' => {
                // -XPOST: rest of the combined token is the method value
            ...
            'H' => {
                // -H needs the next token; rest of combined flags is ignored
                if let Some(raw) = iter.next() {
